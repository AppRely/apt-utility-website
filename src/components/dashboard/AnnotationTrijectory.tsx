...
export default function AnnotationTrajectory() {
  ...
  // 🟢 Load multi-track annotations
  useEffect(() => {
    const loadAnnotations = async () => {
      try {
        const res = await fetch("/shifted.csv")
        if (!res.ok) throw new Error("CSV not found")
        const text = await res.text()
        const lines = text.trim().split("\n").slice(1)
        const parsed: Annotation[] = lines.map((line) => {
          const [frame, x, y, track] = line.split(",").map(Number)
          return { frame, x, y, track: track || 1 }
        })
