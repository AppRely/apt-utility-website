...
type Annotation = { frame: number; x: number; y: number }
...
export default function Annotation() {
  ...
  // --- Load annotations.csv automatically ---
  useEffect(() => {
    const loadAnnotations = async () => {
      try {
        const res = await fetch("/annotations.csv")
        if (!res.ok) throw new Error("CSV not found")
        const text = await res.text()
        const lines = text.trim().split("\n").slice(1)
        const parsed: Annotation[] = lines.map((line) => {
          const [frame, x, y] = line.split(",").map(Number)
          return { frame, x, y }
        })
        setAnnotations(parsed)
        console.log("✅ Loaded annotations:", parsed.length)
      } catch {
        console.warn("⚠️ No annotations.csv found in public folder.")
        setAnnotations([])
      }
    }
    loadAnnotations()
  ...
  const currentAnnotations = annotations.filter(a => a.frame === currentFrame)
